...
class MetricEvaluator:
    ...
    def get_structural_metrics(self, img_source, img_generated):
        """
        Calcule MSE, PSNR, SSIM et Structure Distance entre l'image source
        et l'image générée (mesure de préservation du background).
        Les images sont redimensionnées à la même taille si nécessaire.
        """
        # Conversion PIL → numpy float32 [0, 1]
        ...
        # Structure Distance × 10^3 (combinaison SSIM + gradient, style PIE-Bench)
...
class ComparisonEditor:
    ...
    def process(self, init_image, src_prompt, tgt_prompt, strategy):
        ...
        for i, t_tensor in enumerate(timesteps):
            ...
            current_lambda = base_cfg  # Baseline par défaut
        ...
        for item in DATASET:
            ...
            for tgt_prompt, code in item["targets"]:
                for strat in STRATEGIES:
                    ...
                    # LPIPS (source → générée : mesure de préservation)
                    lpips_d = evaluator.get_lpips_distance(init_img, res_img)

                    # MSE, PSNR, SSIM, Structure Distance
                    mse, psnr, ssim, struct_dist = evaluator.get_structural_metrics(init_img, res_img)

                    print(f"      [{strat:8s}]  CLIP: {clip_s:.4f} | LPIPS: {lpips_d:.4f} | "
                          f"MSE: {mse:.2f} | PSNR: {psnr:.2f} | SSIM: {ssim:.2f} | StructDist: {struct_dist:.2f}")
